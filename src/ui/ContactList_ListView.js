				...
				<div id="users">
					<ul>
						{this.props.contacts.map(person => {
							return <li key={person.id} className="name"><Link to={"/contact/" + person.id}><img className="thumbnail" src={person.thumbnail}/><div className="name__container"><p className="names">{person.fname} {person.lname}</p></div></Link><button className="delete" id={`d${person.id}`} onClick={this.deleteContact}>DELETE</button></li>
						})}
					</ul>
				</div>
			</div>
